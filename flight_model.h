...
public:
    ...
    void operator() (const state_type &x, state_type &dxdt, const double  t) {
        ...
        if (t < 3) {
            ...
            double V_horiz = std::abs(V * std::cos(theta));
            ...
            double dx_t = mission.targetX - X;
            double dy_t = mission.targetY - Y;
            double dz_t = mission.targetZ - Z;
            // Полная дистанция до цели
            double dist_total = std::sqrt(dx_t*dx_t + dy_t*dy_t + dz_t*dz_t);

            // Защита от деления на ноль, если мы уже прилетели
            if (dist_total < 1.0) dist_total = 1.0;
                    double desired_Vy = 0.0;
            // Мы говорим, что желаемая вертикальная скорость должна быть такой,
            // чтобы вектор скорости V был направлен точно на цель.
            if (theta > 0.75)
            {
                // Режим "Вертикальный подъем":
                // Разрешаем использовать 98% скорости для набора высоты
                double max_climb_rate = mission.targetV * 0.98;
                desired_Vy = std::max(-max_climb_rate, std::min(dy_t, max_climb_rate));
            }
            else
            {
                // Режим "Обычный полет по глиссаде":
                desired_Vy = V * (dy_t / dist_total);
            }
            ...
            if (std::abs(c_th) < 1e-3) c_th = 1e-4; // Защита
